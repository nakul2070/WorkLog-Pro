...
const EditClientDialog = ({ open, onOpenChange, client, onClientUpdated }) => {
  ...
  const handleUpdateClient = async () => {
    ...
    const existingClient = clients.find(
      c => c.id !== client.id && c.name.toLowerCase().trim() === clientData.name.toLowerCase().trim()
    );
    if (existingClient) {
      toast({ 
    ...
    try {
      const result = await apiPut(`/api/projects/clients/${client.id}`, { 
      ...
      toast({ 
        title: "Error", 
        description: `Could not update client: ${err.message}`, 
        variant: "destructive" 
      });
    } finally {
